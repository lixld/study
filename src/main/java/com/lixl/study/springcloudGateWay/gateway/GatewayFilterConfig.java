...
import com.lixl.study.springcloudGateWay.business.UserConstant;
import com.lixl.study.springcloudGateWay.util.FilterUtils;
import com.lixl.study.springcloudGateWay.util.RSAConstant;
import com.lixl.study.springcloudGateWay.util.RSAUtils;
...
public class GatewayFilterConfig implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        //1 获取时间戳--url有效期控制
        Long dateTimestamp = getDateTimestamp(httpHeader);

        //2 获取RequestId--只能请求一次
        String requestId = getRequestId(httpHeader);

        //3 获取签名
        String sign = getSign(httpHeader);

        //4 如果是登录不校验Token
        String requestUrl = exchange.getRequest().getPath().value();
        AntPathMatcher pathMatcher = new AntPathMatcher();
        if (!pathMatcher.match("/user/login", requestUrl)) {
            String token = httpHeader.getFirst(UserConstant.TOKEN);
    ...
    public void checkSign(String sign, Long dateTimestamp, String requestId, Map<String, Object> paramMap) {
        ...
        try {
            tempSign = MD5Utils.md5Hex(str.getBytes());
    ...
    private Map<String, Object> updateRequestParam(ServerWebExchange exchange) throws NoSuchFieldException, IllegalAccessException {
        ...
        if (StringUtils.isNotBlank(query) && query.contains("param")) {
            ...
            targetQuery.set(uri, param);//请求参数解密，重新赋值
